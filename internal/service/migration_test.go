...
func TestMigrationService_MigrateGridParams(t *testing.T) {
	ctx := context.Background()
	tester := app.InitializeTestApplication()
	tester.ResetTables(t)
	migrationSrv := tester.MigrationSrv
	...
	t.Run("should migrate grid params", func(t *testing.T) {
		err := migrationSrv.MigrateGridParams(ctx)
		if err != nil {
			t.Fatal(err)
		}

		migratedTasks, err := taskSrv.GetByTradingAccount(ctx, tradingAccount.ID)
		if err != nil {
			t.Fatal(err)
		}
