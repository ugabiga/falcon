...
// QueryTradingAccount chains the current query on the "trading_account" edge.
func (tq *TaskQuery) QueryTradingAccount() *TradingAccountQuery {
	query := (&TradingAccountClient{config: tq.config}).Query()
...
func (tq *TaskQuery) QueryTaskHistories() *TaskHistoryQuery {
...
func (tq *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withTradingAccount: tq.withTradingAccount.Clone(),
		withTaskHistories:  tq.withTaskHistories.Clone(),
...
// WithTradingAccount tells the query-builder to eager-load the nodes that are connected to
// the "trading_account" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TaskQuery) WithTradingAccount(opts ...func(*TradingAccountQuery)) *TaskQuery {
	query := (&TradingAccountClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withTradingAccount = query
	return tq
}
...
func (tq *TaskQuery) WithTaskHistories(opts ...func(*TaskHistoryQuery)) *TaskQuery {
	query := (&TaskHistoryClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withTaskHistories = query
	return tq
}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withTradingAccount; query != nil {
		if err := tq.loadTradingAccount(ctx, query, nodes, nil,
			func(n *Task, e *TradingAccount) { n.Edges.TradingAccount = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withTaskHistories; query != nil {
		if err := tq.loadTaskHistories(ctx, query, nodes,
			func(n *Task) { n.Edges.TaskHistories = []*TaskHistory{} },
			func(n *Task, e *TaskHistory) { n.Edges.TaskHistories = append(n.Edges.TaskHistories, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TaskQuery) loadTradingAccount(ctx context.Context, query *TradingAccountQuery, nodes []*Task, init func(*Task), assign func(*Task, *TradingAccount)) error {
	...
	query.Where(tradingaccount.IDIn(ids...))
